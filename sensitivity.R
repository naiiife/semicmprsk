...
## Dismissible treatment components assumption violated
...
generatedata <- function(N,setting,Z1=1,Z2=1,Z3=1){
  ...
  if (setting==2){ #markov
    ...
    # If Tad=1, T happens; If Tad=0, R happens.
    # Next generate T-R when Tad=0.
    TwR0 = rexp(N,0.05*X1*1/2)
    TwR1 = rexp(N,0.05*(X1+Z3)/2)
  ...
  if (setting==3){ #semimarkov
    ...
    # If Tad=1, T happens; If Tad=0, R happens.
    # Next generate T-R when Tad=0.
    TwR0 = rweibull(N,2,sqrt(2/(0.1*X1)))
    TwR1 = rweibull(N,2,sqrt(2/(0.1*X1+0.1*Z3)))
  }
  R1 = TorR1 + T1d*99
  R0 = TorR0 + T0d*99
  T1 = TorR1 + (1-T1d)*TwR1
...
# A1 and A3 interacting
generatedata2 <- function(N,setting,Z1=1,Z2=1,Z3=1){
  Z10 = Z1+Z3/2
  Z30 = Z1/2+Z3
  Z1 = Z10
  Z3 = Z30
